({
    doInit : function(component, event, helper) {
        ...
        //var getTotal = component.get("c.getTotalDays");
        var getRTypeName = component.get("c.getRecordTypeName");
        var getCase = component.get("c.getCaseDetails");
        ...
        /*getTotal.setParams({"caseId": component.get("v.recordId")});
        getTotal.setCallback(this, function(a){
            console.log('return:' + a.getReturnValue());
            component.set("v.totalDays", a.getReturnValue());
            console.log('totalDays: ' + component.get("v.totalDays"));
            //helper.getDaysOpen(component, event, helper);
        });
        $A.enqueueAction(getTotal);
        */
        getCase.setParams({"caseId" : component.get("v.recordId")});
        getCase.setCallback(this, function(a){
            var caseRec = a.getReturnValue();
            if(caseRec != null){
                component.set("v.myStatus", caseRec.CXE_StatusPath__c);
                component.set("v.totalDays", caseRec.CXE_Total_Days_Open__c);
                //START CSM-12156 RReyes AUG-04-17 - added status field
                component.set("v.caseStatus", caseRec.Status);
                helper.buildPath(component, event, helper);
                //END CSM-12156 RReyes AUG-04-17 - added status field
            }
            console.log('rr>> my status: ' + component.get("v.myStatus"));
            console.log('rr>> my totalDays: ' + component.get("v.totalDays"));
            console.log('rr>> my case Status: ' + component.get("v.caseStatus"));
            console.log('new code applied');
        });
        $A.enqueueAction(getCase);
        
        getRTypeName.setParams({"caseId": component.get("v.recordId")});
        getRTypeName.setCallback(this, function(a){
            console.log('return:' + a.getReturnValue());
            component.set("v.rType", a.getReturnValue());
            console.log('rType: ' + component.get("v.rType"));
            //helper.getDaysOpen(component, event, helper);
        });
        $A.enqueueAction(getRTypeName);
