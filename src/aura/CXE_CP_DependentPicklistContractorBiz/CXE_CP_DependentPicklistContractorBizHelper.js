({
   ...
   handleEvent : function(component, event, helper, eventName){
      ...
      if(eventName == "CXE_selectBusinessProvinceEvent"){
         selectEvent = component.getEvent("businessProvinceContractor");
         
         selectEvent.setParams({ "businessProvinceContractor": event.getSource().get("v.value") }).fire();
         
      } else if (eventName == "CXE_selectBusinessCityEventContractor"){
         selectEvent = component.getEvent("businessCityContractor");
         
         selectEvent.setParams({ "businessCityContractor": event.getSource().get("v.value") }).fire();
         
      } else if (eventName == "CXE_selectControllingPicklistEventContractor"){
         selectEvent = component.getEvent("serviceProvinceContractor");
         selectEvent.setParams({ "controllingPicklist": event.getSource().get("v.value") }).fire();
      } else if (eventName == "CXE_selectDependentPicklistEventContractor"){
         selectEvent = component.getEvent("serviceCityContractor");
         selectEvent.setParams({ "dependentPicklist": event.getSource().get("v.value") }).fire();
      } 
   },
   ...
   proceedChangeHelper: function(component, event, helper) {
      ...
      var flagCon = false; 
      var flagDep = false; 
      var compareString = "Select One";
      
       if($A.util.isEmpty(controllingFieldId) || controllingFieldId == compareString){
       ...
       }if($A.util.isEmpty(dependentFieldId) || dependentFieldId == compareString){
